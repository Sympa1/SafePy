...
class ErstelleMasterAccount(MasterAccount):
    ...
    def erstelle_account(self):
        ...
        sql_daten = (str(self._dienst), self._salt, str(self._nutzername), str(self._passwort_hash))
...
class LoginMasterAccount(MasterAccount):
    ...
    def login_account(self):
        """Eine Methode zum Login. Diese Überprüft das Passwort des Master-Accounts"""
        # TODO: Eine ÜBerprüfung des Nutzernamens inplementieren
        datenbank = DatenbankMaster(self._nutzername)
        datenbank.db_connection_herstellen()
        datenbank_cursor =  datenbank.db_cursor()
        ...
        self._passwort_gespeichert_hash = rueckgabewerte[0]
        self._salt = rueckgabewerte[1]

        self.erstelle_hashwert()

        if  self._passwort_hash == self._passwort_gespeichert_hash:
            return True
        else:
            return False